export const scheduleDay1 = [
  { time: "09:00 AM", title: "OPENING", description: "Welcome to Entangle and introduction to the Odyssey." },
  { time: "10:30 AM", title: "QUANTUM FUNDAMENTALS", description: "The transition from classical bits to quantum qubits." },
  { time: "01:00 PM", title: "QUANTUM MECHANICS", description: "Understanding the underlying physics." },
  { time: "03:00 PM", title: "SUPERPOSITION", description: "Exploring parallel solution spaces." },
  { time: "05:00 PM", title: "ENTANGLEMENT", description: "The spooky action at a distance." },
];

export const scheduleDay2 = [
  { time: "09:00 AM", title: "QUANTUM CIRCUITS", description: "Exploring logic gates for qubits." },
  { time: "11:00 AM", title: "ALGORITHMS", description: "Shor's and Grover's algorithms explained." },
  { time: "02:00 PM", title: "HANDS-ON EXPERIENCE", description: "Interactive session with quantum simulators." },
  { time: "04:00 PM", title: "APPLICATIONS", description: "Real-world impact of quantum computing." },
  { time: "05:30 PM", title: "EXPERT INTERACTION", description: "Q&A with researchers and closing ceremony." },
];
